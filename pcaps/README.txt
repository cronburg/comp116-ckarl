...
1. How many packets are there in this set?
	A: Set 1 has 276 packets.

2. What protocol was used to transfer files from PC to server?
	A: File Transfer Protocol (FTP)

3. Briefly describe why the protocol used to transfer the files is insecure?
	A: The protocol is insecure because it transmits all data (username, password,
	and the actual files) in clear-text over the wire. Someone sniffing the
	network can readily obtain (username,password) pairs, or a MitM attack
	can be employed to alter the files / data being transmitted.

4. What is the secure alternative to the protocol used to transfer files?
	A: SFTP is the secure alternative to file transfer, which transmits usernames,
	passwords, and files with SSH or TLS.

5. What is the IP address of the server?
	A: 67.23.79.113

6. What was the username and password used to access the server?
	A: (username,password) = (stokerj,w00tfu!)

7. How many files were transferred from PC to server?
	A: Three files.

8. What are the names of the files transferred from PC to server?
	A: "code.rtf", "secret.pdf", and "acb.jpg"
...
the corresponding domain name (e.g., google.com), and port number.
	A: See table above. Both server and client port numbers are included.
...
username-password pairs that you found.
	A: It is a crime to attempt to login to a website or service using someone's account without
	their permission.
...
are in this PCAP set. Describe your methodology.
	A: The "Statistics -> Show address resolution" functionality of wireshark is used to produce the
	following list of IP-name pairs. The hostname is the first portion of the FQDN and the domain
	name is the last part, e.g. for the first one the hostname is "talkgadget.l" and the domain
	name is "google.com". One could also use the nslookup or whois commands to determine FQDNs.
...
19. Provide a summary of the all protocols used. What was the most popular
protocol used? Describe your methodology.
	A: The "Statistics -> Protocol Hierarchy" is used to determine what
	percentage of the number of packets in the PCAP set used each protocol. The most common
	protocols fell under "Frame->Ethernet->IPv4" with 96.5% of packets using TCP,
	1.46% using UDP, and 1.28% using ICMP and P2PP. The remaining 0.35% used IPv6.

	The most popular application-level protocols used (falling under "IPv4->TCP")
	were Data (7.58%), SSL (7.39%), HTTP (8.78%), and FTP Data (11.45%).
	
	All protocols used include:
	Data, SSL, HTTP, FTP, telnet, POP, FTP Data, SSH, IMAP, XMPP, DNS,
	OpenVPN, NTP, Bootstrap, NetBIOS Name Service, SIP, ISAKMP, Dropbox LAN Discovery,
	P2PP, ICMP, ARP, IPv4, TCP, UDP, IPv6.
	These were obtained using the "Protocol Hierarchy Statistics" function in wireshark.

20. A fun question: what other interesting things did you find in this PCAP set (e.g., files)?
	I found various PNGs, JPGs, PDFs, CSS scripts, GIFs, XML files, JS files, and more using
	wireshark's export functionalitiy. Some of the images were clearly from facebook
	chat. Other images were thumbnails, in particular a group of photos from the Grand Canyon.
